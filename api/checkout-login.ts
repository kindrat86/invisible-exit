...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const user = await queryOne<AppUser>(
      `INSERT INTO app_users (email, password_hash, stripe_customer_id, subscription_status, subscription_tier)
       ...
       DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING id, email, stripe_customer_id, subscription_status, subscription_tier`,
      [email, passwordHash, stripeCustomerId, tier]
    );
